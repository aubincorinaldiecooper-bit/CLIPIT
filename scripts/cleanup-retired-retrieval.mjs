...
await edit('src/db/repositories/videos.ts', (s) => s
  ...
  .replace(/\n\/\*\*\n \* A video with how much of it its notes describe\.[\s\S]*?export async function getVideoWithReadProgress\(videoId: string\): Promise<Video \| null> \{[\s\S]*?\n}\n/, '\n'));

await edit('src/api/serializers.ts', (s) => s
  .replace(/\n    index: \{\n      status: video\.indexStatus,[\s\S]*?\n    \},\n    createdAt:/, '\n    createdAt:'));

await edit('src/api/routes/videos.ts', (s) => s
  .replace('  getVideoWithReadProgress,\n', '')
  .replace(/    \/\/ This route is a pure read\.[\s\S]*?    const video = await getVideoWithReadProgress\(videoId\);/, `    // This route is a pure read. SimpleMem progress belongs to retrieval, not\n    // a retired upload-time notes index, so status comes straight from the video.\n    const video = await getVideo(videoId);`)
...
await unlink('scripts/cleanup-retired-retrieval.mjs');
await unlink('.github/workflows/cleanup-retired-retrieval.yml');

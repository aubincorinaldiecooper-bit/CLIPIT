...
export interface UnknownRender {
  ...
  /** When the render was written down — after its re-reads and their retries. What a record from before `rowUpdatedAt` is read against. */
  recordedAt: Date;
  /**
   * The clip row's updated_at as the render's attempt wrote it when it set
   * the row generating — its last write of its own before the one whose
   * outcome was lost. A row written after this was written by that write or
   * by something later. Null on records from before migration 037.
   */
...
export async function recordUnknownRender(input: Omit<UnknownRender, 'id' | 'recordedAt'>): Promise<void> {
  await queryOne(
    `INSERT INTO unknown_renders (clip_id, storage_key, previous_storage_key, row_updated_at, job)
     VALUES ($1, $2, $3, $4, $5::jsonb)
     RETURNING id`,
    [input.clipId, input.storageKey, input.previousStorageKey, input.rowUpdatedAt, JSON.stringify(input.job)],
  );
}
...
export async function drainUnknownRenders(
  ...
  return withTransaction(async (client) => {
    const { rows } = await client.query<{ id: string; clip_id: string; storage_key: string; previous_storage_key: string | null; row_updated_at: Date | null; job: ClipGenerationJob; created_at: Date }>(
      `SELECT id, clip_id, storage_key, previous_storage_key, row_updated_at, job, created_at
    ...
    for (const row of rows) {
      await settle({
        ...
        job: row.job, recordedAt: row.created_at, rowUpdatedAt: row.row_updated_at,
      }, client);

...
const row = { id: 'ur-1', clip_id: 'clip-1', storage_key: 'clips/video-1/clip-1-0ddba11a.mp4', previous_storage_key: 'clips/video-1/clip-1.mp4', job: { clipId: 'clip-1' }, created_at: recordedAt };
...
describe('drainUnknownRenders', () => {
  it('settles each row with the transaction\'s client, then deletes it', async () => {
    ...
    expect(settle).toHaveBeenCalledWith(
      { id: 'ur-1', clipId: 'clip-1', storageKey: row.storage_key, previousStorageKey: row.previous_storage_key, job: row.job, recordedAt },
      client,
    );

...
describe('createHandoff', () => {
  ...
  it('sweeps expired claims and keeps only this guest’s newest few live ones, before adding another', async () => {
    queryOne.mockResolvedValueOnce({ expires_at: new Date() });

    await createHandoff('guest-session', 'a@b.c');

    const statements = query.mock.calls.map((call) => [String(call[0]), call[1]] as [string, unknown[] | undefined]);
    expect(statements[0]![0]).toMatch(/DELETE FROM session_handoffs WHERE expires_at <= now\(\)/);

...
const live: FrameStreamVideoSource = {
  ...
  id: 'page-1',
  async *open() {
    yield { timestampMs: 1_250, encoding: 'jpeg', image: Buffer.from('frame') };
  },
};
...
describe('video source/model ports', () => {
  it('passes a compatible source and query through without source-specific orchestration', async () => {
    const watch = vi.fn(async () => ({
      ...
      revision: 'r1',
      durationSeconds: 10,
      watchedThroughSeconds: 10,
      moments: [{ startSeconds: 1, endSeconds: 2, description: 'match' }],
      metrics: {},
    }));
    const model: VideoModelAdapter = {
      id: 'fake',
  ...
  it('fails before a model call when the source representation is incompatible', async () => {
    ...
    await expect(watchVideo({ model, source: live, query: 'find it' })).rejects.toThrow(
      'file-only cannot read video source kind "frame-stream"',
    );
  ...
  it('keeps verification behind the same model/source boundary', async () => {
    ...
    const result = await verifyVideo({
      ...
      source: stored,
      query: 'find it',
      candidates: [{ id: 'candidate-1', start: 3, end: 4 }],
    });
  ...
  it('does not pretend every watcher supports a second verification pass', async () => {
    ...
    await expect(verifyVideo({
      model,
      source: stored,
      query: 'find it',
      candidates: [],
    })).rejects.toThrow('watch-only does not expose interval verification');
  });
});

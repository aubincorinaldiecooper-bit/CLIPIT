...
3. When memory has no verified answer, the footage is read the way an internet video is read: VideoChat3 watches the analysis proxy for the question, Qwen embeddings and the Qwen reranker order what it flagged, and VideoChat3 re-opens each candidate before it becomes evidence. A watch that verified nothing completes the request; the stretch after the watch's event cap, if it is reached, is recorded as unexamined.
   What a candidate must satisfy is decided once, when the request's mode is resolved, and recorded on the request (`resolved_mode`, `resolved_evidence`). A question that mixes spoken and visual conditions, or that asks for both by name, needs footage and the timestamp-aligned transcript of the same interval judged together; a silent interval is rejected, never verified on the picture instead. An undetermined question, or a quoted phrase that may be spoken or on screen, is searched in both sources and either may establish a moment, which is then labelled by what established it; for those questions speech proposes too (a quoted phrase looked up in the timestamped transcript, otherwise the retained transcript-only per-chunk search, which sends words and never video), and every spoken proposal is re-opened by VideoChat3 with its footage and its aligned transcript before it is evidence. Nothing downstream re-reads the question to decide this.
